...
class ScaleFactorBase(object):
    ...
    def addSFObject(self,tag,objs , stats = None , systs = None):

        """ adds an object directory  """

        self.sf_dict[tag]={'main':objs, 'stat':stats , 'syst':systs}
	for obj in objs:
    ...
    def addSFFromSource(self,tag,url,obj):
        ...
        for u in url:
            u=ROOT.gSystem.ExpandPathName(u)
            if not os.path.isfile(u) : 
                raise IOError(errno.ENOENT, os.strerror(errno.ENOENT), u)

        fIn=[ROOT.TFile.Open(u) for u in url]
        sfObj =[ f.Get(obj[i]) for i,f in enumerate(fIn) ]
        statObj =[ f.Get( obj[i] + "_stat" ) for i,f in enumerate(fIn) ]
        systObj =[ f.Get( obj[i] + "_syst" ) for i,f in enumerate(fIn) ]
#        print (tag, url , obj, sfObj, statObj, systObj)
	...
	if 'hlt' in tag:
        ...
        sfVal=None
        sfObj=self.sf_dict[tag]['main'][0] if 'hlt' not in tag 
...
#            print('evalSFs',self.sf_dict)
    ...
    def evalSF_hlt(self,tag,objs):
        ...
        sfVal=None
        sfObj_effMC1 = self.sf_dict[tag]['main'][0]
        sfObj_SF1 = self.sf_dict[tag]['main'][1]
        sfObj_effMC2 = self.sf_dict[tag]['main'][2]
        sfObj_SF2 = self.sf_dict[tag]['main'][3]
        if sfObj_effMC.InheritsFrom('TH1') and sfObj_SF.InheritsFrom('TH1'):

            if sfObj_effMC.InheritsFrom('TH2') and sfObj_SF.InheritsFrom('TH2'):
               eff_ev_mc = sfObj_effMC1.GetBinContent(sfObj_effMC1.FindBin(objs[0].pt,objs[0].eta))*fObj_effMC2.GetBinContent(sfObj_effMC2.FindBin(objs[1].pt,objs[1].eta))    
        return sfVal
