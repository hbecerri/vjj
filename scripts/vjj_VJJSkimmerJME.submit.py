...
def main():
    ...
    if not os.path.exists( opt.logdir ): os.makedirs( opt.logdir )
        ...
        if opt.includeexistingfiles:
            ...
            for step in range( nsteps ):
                ...
                if not exists or os.path.getsize(outfilepath)==0: #Will resubmit job if outfile does not exist OR size==0 (changed)
                    ...
                    try:
                        inputfilenames = get_fileNames(campaign, s, opt.nfilesperchunk, step, True) #Prints: (dataset,nfilesperchunk,chunkindex,nfiles,nchunks)
